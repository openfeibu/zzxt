			...
			<div class="col-xs-12">
				<form class="form-horizontal memberform" name="member_list_edit" method="post" action="<?php echo url('admin/Member/member_runedit'); ?>">
					...
					<div class="form-group">
						...
						<div class="col-sm-10">
							<input type="text" name="member_list_nickname" id="member_list_nickname" value="<?php echo $member_list_edit['member_list_nickname']; ?>"  placeholder="输入姓名" class="col-xs-10 col-sm-4" />
						</div>
					</div>
					<div class="space-4"></div>
					...
					<div class="form-group" style="display: none">
						...
						<div class="col-sm-10">
							<select name="member_list_province" id="province" onChange="loadRegion('province',2,'city','<?php echo url('admin/Ajax/getRegion'); ?>');">
								<option value="0">省份/直辖市</option>
								<?php if(is_array($province) || $province instanceof \think\Collection || $province instanceof \think\Paginator): $i = 0; $__LIST__ = $province;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
									<option value="<?php echo $vo['id']; ?>" <?php if($member_list_edit['member_list_province'] == $vo['id']): ?>selected<?php endif; ?>><?php echo $vo['name']; ?></option>
								<?php endforeach; endif; else: echo "" ;endif; ?>
							</select>
							<select name="member_list_city" id="city"  onchange="loadRegion('city',3,'town','<?php echo url('admin/Ajax/getRegion'); ?>');">
								<option  value="0">市/县</option
								<?php if(is_array($city) || $city instanceof \think\Collection || $city instanceof \think\Paginator): if( count($city)==0 ) : echo "" ;else: foreach($city as $key=>$v): ?>
									<option <?php if($member_list_edit['member_list_city'] == $v['id']): ?>selected<?php endif; ?> value="<?php echo $v['id']; ?>"><?php echo $v['name']; ?></option>
								<?php endforeach; endif; else: echo "" ;endif; ?>

							</select>
							<select name="member_list_town" id="town">
								<option value="0">镇/区</option>
								<?php if(is_array($town) || $town instanceof \think\Collection || $town instanceof \think\Paginator): $i = 0; $__LIST__ = $town;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vy): $mod = ($i % 2 );++$i;?>
									<option <?php if($member_list_edit['member_list_town'] == $vy['id']): ?>selected<?php endif; ?> value="<?php echo $vy['id']; ?>"><?php echo $vy['name']; ?></option>
								<?php endforeach; endif; else: echo "" ;endif; ?>
							</select>
						</div>
					</div>
