;(function(window, $) {
	var regexs = {
	    ...
	    * 国际码 如：中国(+86)
	    */
	    phone:/^((\+?[0-9]{1,4})|(\(\+86\)))?(13[0-9]|14[579]|15[012356789]|17[01235678]|18[0-9]|19[89])\d{8}$/,
	    /**
	     * @descrition 匹配 URL
	     */
	    url:/[a-zA-z]+:\/\/[^\s]/
	...
	fbUi.expPhone = function(test){
		var test = test;
	    if(!(/^1[345678]\d{9}$/.test(test))){
	        return "格式不正确";
	    }
	    return true;
	};
	fbUi.expPhoneTel = function(test){
		var test = test;
	    if(!(/^1[345678]\d{9}$/.test(test)) && !(/^(?:(?:0\d{2,3})-)?(?:\d{7,8})(-(?:\d{3,}))?$/.test(test))){
	        return "格式不正确";
	    }
	    return true;
	};
	...
	//正则:验证正数
	fbUi.expPositiveNumber = function(test){
		var test = test;
	    if(!(/^\d+(\.\d+)?$/.test(test))){
			return '格式不正确';
	    }
	...
	//正则:验证字数最小
	fbUi.expMin = function(test,number){
		var test = test;
		var number = number;
		if(test.length < number)
		{
			return '字数不足';
		}
	    return true;
	...
	//正则:验证字数最大
	fbUi.expMax = function(test,number){
		var test = test;
		var number = number;
		if(test.length < number)
		{
