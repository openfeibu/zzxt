...
function check_user_action($object="",$count_limit=1,$ip_limit=false,$expire=0)
{
    ...
    if($find_log){
        ...
        if (config('database.type')=='sqlsrv'){
            Db::name("action_log")->insert(array("uid"=>$userid,"action"=>$action,"object"=>$object,"count"=>1,"last_time"=>$time,"ip"=>$ip));
        } else {
...
{
	$score = 0;
	$eval_score_config = Config::get('evaluation');
	if(isset($eval_app['establish_card'])){
		$score += $eval_score_config['establish_card'][$eval_app['establish_card']];
	}
	if(isset($eval_app['special_person'])){
		$score += $eval_score_config['special_person'][$eval_app['special_person']];
	}
	if(isset($eval_app['mini_living'])){
		$score += $eval_score_config['mini_living'][$eval_app['mini_living']];
	}if(isset($eval_app['establish_card'])){
		$score += $eval_score_config['establish_card'][$eval_app['establish_card']];
	}
	if(isset($eval_app['poor_children'])){
		$score += $eval_score_config['poor_children'][$eval_app['poor_children']];
	}
	if(isset($eval_app['low_income'])){
		$score += $eval_score_config['low_income'][$eval_app['low_income']];
	}
	if(isset($eval_app['orphan'])){
		$score += $eval_score_config['orphan'][$eval_app['orphan']];
	}
	if(isset($eval_app['martyrs_children'])){
		$score += $eval_score_config['martyrs_children'][$eval_app['martyrs_children']];
	}
	if(isset($eval_app['natural_disasters_type'])){
		$score += $eval_score_config['natural_disasters_type'][$eval_app['natural_disasters_type']];
	}
	if(isset($eval_app['unexpected_events_type'])){
		$score += $eval_score_config['unexpected_events_type'][$eval_app['unexpected_events_type']];
	}
	if(isset($eval_app['disability'])){
		$score += $eval_score_config['disability'][$eval_app['disability']];
	}
	if(isset($eval_app['single_parent'])){
		$score += $eval_score_config['single_parent'][$eval_app['single_parent']];
	}
	if(isset($eval_app['account'])){
		$score += $eval_score_config['account'][$eval_app['account']];
	}
	if(isset($eval_app['residence_address_situation'])){
		$score += $eval_score_config['residence_address_situation'][$eval_app['residence_address_situation']];
	}
	if(isset($eval_app['car_situation']) && isset($eval_app['housing_situation'])){
		$house_assests = 0;
		if($eval_app['car_situation'] == 0 && $eval_app['housing_situation'] != 0){
			$house_assests = 1;
		}
		$score += $eval_score_config['house_assests'][$house_assests];
	}
	if(isset($eval_app['annual_income'])){
		if($eval_app['annual_income'] <= $eval_score_config['annual_income']['min1']['value'])
		{
			$score += $eval_score_config['annual_income']['min1']['score'];
		}else if($eval_app['annual_income'] <= $eval_score_config['annual_income']['min2']['value'])
		{
	...
	if(isset($eval_app['school_population'])){
		...
		{
			$score += $eval_score_config['school_population']['min']['score'];
		}
	}
...
{
	if (isset($apply['group_opinion']) && !empty($apply['group_opinion'])) {
	...
	if (isset($apply['school_opinion']) && !empty($apply['school_opinion'])) {
		...
		else if($apply['school_poor_grade'] && $apply['school_poor_grade'] == $apply['faculty_poor_grade'])
		{
			$apply['school_opinion']['text'] = '同意学院评级';
		}else{
...
function handleScholarshipApply($apply = array()){
	if(isset($apply['awards']) && !empty($apply['awards'])) {
	...
	if (isset($apply['members']) && !empty($apply['members'])) {
