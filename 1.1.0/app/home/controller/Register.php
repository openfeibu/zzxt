...
class Register extends Base
{
    ...
    public function active()
    {
		$hash=input('hash','');
		if(empty($hash)){
			$this->error(lang('hash incorrect'));
		}
		$users_model=Db::name("member_list");
		...
		if($find_user){
			...
			if($result){
				...
				$this->success(lang('active success'),url('home/student/examinestatus'));
			}else{
				$this->error(lang('active failed'),url("home/Login/index"));
			}
		}else{
			$this->error(lang('hash incorrect'),url("home/Login/index"));
		}
	}

	/**
	 * 导入学生资料（最好一次500个导入，不然会超时）
     * 密码加密
	 */
	 public function Registerhahah() {
         $data = Db::table('yf_user')
             ->where('id', 'BETWEEN','9001,9604')
             ->select();
         foreach ($data as $vo) {
             $member_list_username = $vo['studentid'];
             $password = $vo['password'];
             $member_list_nickname = $vo['studentname'];
             $member_list_salt = random(10);
             $member_list_email = ' ';
             $sl_data = array(
                 'member_list_username' => $member_list_username,
