...
class Base extends Common
{
	...
	protected function _initialize()
    {
		...
		if(session('hid')){
			...
			$this->user=Db::name('member_list')->alias('m')->join('yf_user u','u.id_number = m.id_number')->where('m.member_list_id',session('hid'))->field('u.*,m.member_list_id,m.member_list_username,member_list_email,m.member_list_province,m.member_list_city,m.member_list_town,m.member_list_nickname,m.member_list_headpic,m.user_status')->find();
		...
		$is_question_open = DB::table('yf_question_switch')
            ->where('is_open',1)
            ->find();
		$this->assign("is_question_open",$is_question_open);
	}
	/**
	 * 检查用户登录
	 */
	...
	protected function check_active()
	{
		...
		// if(!$this->user['user_status']){
			// $this->redirect(url('home/Center/active'));
		// }
	}
	/**
	 * 检查操作频率
