...
<div class="page-content">
    ...
    <input type="hidden" name="faculty_number" value="<?php echo $faculty; ?>">
    <div class="row clearfix">
        <div class="col-md-12 column">
            <div>
                <div class="personal_num" style="width: 800px;">
                    <div class="passed">
                        <p class="success_bg"><span><?php echo $faculty_name; ?></span>已评定人数：</p>
                        <p class="default_bg"><span><?php echo $faculty_pass; ?></span>人</p>
                    </div>
                    <div class="passed pull-right">
                        <p class="danger_bg"><span><?php echo $faculty_name; ?></span>未评定人数：</p>
                        <p class="default_bg"><span><?php echo $faculty_not_pass; ?></span>人</p>
                    </div>
                </div>
                <div class="clearfix"></div>
                <form class="tab_inquire" method="post" action="">
                    <p>筛选：</p>
                    <select class="form-control tab_select" style="width:15%;" name="grade">
                        <option value="0"></option>
                        <option value="2015">2015级</option>
                        <option value="2016">2016级</option>
                        <option value="2017">2017级</option>
                    </select>
                    <select class="form-control tab_select manage_select" name="faculty">
                        <option value="0"><a href="">全系</a></option>
                        <?php foreach($profession as $f): ?>
                        <option value="<?php echo $f['profession_number']; ?>"><?php echo $f['profession']; ?></option>
                        <?php endforeach; ?>
                    </select>
                    <button class="btn btn-info btn-sm" style="margin-bottom: 3px;">查询</button>
                ...
                <!--<table class="table table-striped table-bordered table-hover tab_center">
                    ...
                    <th class="info_bg">评定状态</th>
                    <th class="info_bg">操作</th>
                    </thead>
                    <tbody>
                    <?php $num = 1;foreach($user as $row): ?>
                    <tr>
                        <td class="tab_content"><?php echo $num++; ?></td>
                        <td class="tab_content"><?php echo $row['studentname']; ?></td>
                        <td class="tab_content"><?php echo $row['studentid']; ?></td>
                        <td class="tab_content">
                            <?php if($row['status'] == 1): ?>申请书已经提交
                            <?php elseif($row['status']==2): ?>班级小组通过/辅导员通过
                            <?php elseif($row['status']==3): ?>院系小组通过
                            <?php elseif($row['status']==4): ?>学生处通过
                            <?php elseif($row['status']==5): ?>班级小组不通过
                            <?php elseif($row['status']==6): ?>院系小组不通过
                            <?php elseif($row['status']==7): ?>学生处不通过
                            <?php endif; ?>
                        </td>
                        <td class="tab_content">
                            <div class="audit_opera">
                                <a href="<?php echo url('admin/FacultyGroup/showMaterial',['id'=>$row['status_id'],'type_id'=>$type_id]); ?>" class="btn btn-info btn-sm ">通过</a>
                            </div>
                            <div class="audit_opera">
                                <a href="<?php echo url('admin/FacultyGroup/showMaterial',['id'=>$row['status_id'],'type_id'=>$type_id]); ?>" class="btn btn-info btn-sm">不通过</a>
                            </div>
                            <div class="audit_opera">
                                <a href="<?php echo url('admin/FacultyGroup/showMaterial',['id'=>$row['status_id'],'type_id'=>$type_id]); ?>" class="btn btn-info btn-sm">查看</a>
                            </div>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                ...
                <table id="simple-table"  class="table table-striped table-bordered table-hover tab_center">
                    ...
                    <tr>
                        ...
                        <td class="tab_content">
                            <div class="audit_opera">
                                <a href="<?php echo url('admin/FacultyGroup/showMaterial',['id'=>$row['status_id'],'type_id'=>$type_id]); ?>" class="btn btn-info btn-sm btn-success">通过</a>
                            </div>
                            <div class="audit_opera">
                                <a href="<?php echo url('admin/FacultyGroup/showMaterial',['id'=>$row['status_id'],'type_id'=>$type_id]); ?>" class="btn btn-info btn-sm btn-danger">不通过</a>
                            </div>
                            <div class="audit_opera">
                                <a href="<?php echo url('admin/FacultyGroup/showMaterial',['id'=>$row['status_id'],'type_id'=>$type_id]); ?>" class="btn btn-info btn-sm">查看</a>
                            </div>
